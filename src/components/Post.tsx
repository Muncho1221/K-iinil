...
export const Post = ({ post, onToggleLike, onToggleSave, onToggleComment, isCommentOpen, commentText, setCommentText }: any) => {
  return (
    <article className="bg-white rounded-2xl border border-lavender-100 overflow-hidden mb-4">
      {/* Post header */}
      <div className="flex items-center gap-3 px-4 pt-4 pb-3">
        <img src={post.avatar} alt={post.user} className="w-10 h-10 rounded-full object-cover border-2 border-plum-200" />
        <div className="flex-1">
          <p className="text-sm font-semibold text-plum-900">{post.user}</p>
          <p className="text-xs text-plum-400">{post.handle} · {post.time}</p>
        </div>
      ...
      <div className="px-4 py-3 flex items-center gap-4">
        ...
        >
          ...
          <span className={`font-medium ${post.liked ? 'text-pink-500' : ''}`}>{post.likes.toLocaleString()}</span>
        </button>
        <button
